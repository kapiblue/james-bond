...
```{r kills, fig.width= 14, fig.height= 8, plotly=TRUE}

plot_df <- JamesBond
plot_df$Movie <- factor (plot_df$Movie , levels = plot_df$Movie) # we want to preserve the dataframe's chronological movie ordering

fig2 <- plot_ly(plot_df, x = ~Year, y = ~Kills_Bond, name = 'Kills Bond', type= 'scatter', mode= 'lines+markers',
...
fig2
```
```{r corr, fig.width = 14, fig.height = 8, warning=FALSE}

fig_corr <- JamesBond %>% select(-US_Gross, -World_Gross, -US_Adj, -Budget, -Avg_User_Rtn_Tom, -Movie, -Bond) %>%
  relocate(Avg_User_IMDB, .after = last_col()) %>%
  cor() %>% 
  ggcorrplot(method = 'circle', type = 'upper', outline.color = 'white', lab_size = 8,
