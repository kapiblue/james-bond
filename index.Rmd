...
```{r martinis, echo=FALSE, fig.width= 14, fig.height= 8}
plot_df <- JamesBond
plot_df$Movie <- factor (plot_df$Movie , levels = plot_df$Movie) # we want to preserve the dataframe's chronological movieordering


g <- ggplot(plot_df, aes(Movie, Martinis, group = 1)) + geom_line(size = 1.5) + geom_point(size = 5) +
  labs(
    title = "The number of Martinis Bond drank in the film"
  )
  
style <- theme_classic() +
  theme(
    axis.text.x = element_text(angle = 35, margin = margin(t=50)),
